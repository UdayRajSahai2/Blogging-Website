...
const NotificationCard = ({ data, index, notificationState, refreshNotifications }) => {
    ...
    const handleDeleteNotification = async () => {
        ...
        try {
            const response = await axios.delete(
                `${import.meta.env.VITE_SERVER_DOMAIN}/delete-notification`,
                {
            ...
            if (response.data.success) {
                toast.success(isOwnReply ? "Notification hidden" : "Notification deleted");
                
                const { notifications, setNotifications } = notificationState;
                
                if (notifications && notifications.results) {
                    const updatedResults = notifications.results.filter((_, i) => i !== index);
    ...
    const handleDeleteComment = async () => {
        if (!confirm("Delete this reply permanently? This will remove it from the post and all notifications.")) {
            return;
        }
    ...
    return (
        <div className={`p-6 border-b border-grey transition-all duration-200 ${
            isOwnReply 
                ? 'bg-grey/5 border-l-4 border-l-purple/30' 
                : 'border-l-4 border-l-black'
        }`}>
            <div className="flex gap-5 mb-3">
                <img src={profile_img} className="w-14 h-14 flex-none rounded-full" alt={fullname} />
                <div className="w-full">
                    <h1 className={`font-medium text-xl ${isOwnReply ? 'text-dark-grey/90' : 'text-dark-grey'}`}>
                        {isOwnReply && (
                        ...
                        <Link to={`/user/${username}`} className={`mx-1 underline ${isOwnReply ? 'text-purple' : 'text-black'}`}>
                            @{username}
                        </Link>
                        <span className="font-normal">
                            {
                                ...
                                isOwnReply ? "your reply on" : "replied on"
                            }
                        </span>
                    </h1>
                    {
                        type === "reply" ? 
                        <div className={`p-4 mt-4 rounded-md ${isOwnReply ? 'bg-purple/5 border border-purple/20' : 'bg-grey'}`}>
                            <p className="text-sm text-dark-grey mb-2 font-medium">Original comment:</p>
                            <p className="text-dark-grey/80">{original_comment ? original_comment.comment : "Original comment not found"}</p>
                        </div>
                        : 
                        <Link to={`/blog/${blog_id}`} className={`font-medium hover:underline line-clamp-1 ${isOwnReply ? 'text-purple' : 'text-dark-grey'}`}>
                            {`"${title}"`}
                        </Link>
                    }
                </div>
            ...
            {
                type !== 'like' ?
                <div className={`ml-14 pl-5 font-gelasio text-xl my-5 ${
                    isOwnReply ? 'border-l-2 border-purple/30 bg-purple/5 p-4 rounded-r-md' : ''
                }`}>
                    {isOwnReply && (
