          ...
          return (
            <div key={index} className="space-y-1">
              <div className="flex justify-between text-sm">
                ...
                <span className="font-semibold text-emerald-600">₹{parseFloat(item[yKey]).toLocaleString()}</span>
              </div>
        ...
        <svg width="100%" height="100%" className="overflow-visible">
          ...
          <polyline
            ...
            points={data.map((item, index) => {
              ...
              const y = chartHeight - (item.cumulative / maxCumulative) * chartHeight;
              return `${x}%,${y}`;
            }).join(' ')}
          />
...
const DonationAnalyticsCharts = ({ selectedYear }) => {
  ...
  if (!analytics || analytics.donationData.length === 0) {
    return (
      <div className="bg-white border border-grey rounded-xl p-6">
        <h3 className="text-lg font-semibold text-dark-grey mb-4">Donation Analytics</h3>
        ...
        <div className="bg-gradient-to-br from-emerald-50 to-green-50 border border-emerald-200 rounded-xl p-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xl font-bold text-emerald-700">₹{analytics.totalDonated.toLocaleString()}</p>
            </div>
          </div>
        </div>
        ...
        <div className="bg-gradient-to-br from-blue-50 to-indigo-50 border border-blue-200 rounded-xl p-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xl font-bold text-blue-700">{analytics.donationData.length}</p>
            </div>
          </div>
        </div>
