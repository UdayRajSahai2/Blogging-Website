...
const importLocationData = async () => {
  try {
    ...
    const villageDataPath = path.join(
      __dirname,
      "../data/village-lgd-codes.json",
    );
    ...
    if (!Array.isArray(fieldNames) || !Array.isArray(records)) {
      throw new Error(
        'Invalid JSON structure. Expected "fields" and "records" arrays.',
      );
    }

    // Map each record array to an object using field names
    const mappedRecords = records.map((arr) =>
    ...
    mappedRecords.forEach((record, index) => {
      ...
      if (stateCode && stateName && countryCode) {
        uniqueData.states.add(
          JSON.stringify({ stateCode, stateName, countryCode }),
        );
      } else if (index < 3) {
        console.log("Skipping state for record", index + 1, {
          stateCode,
          stateName,
          countryCode,
        });
      ...
      if (districtCode && districtName && stateCode && countryCode) {
        ...
        console.log("Skipping district for record", index + 1, {
          districtCode,
          districtName,
          stateCode,
          countryCode,
      ...
      if (blockCode && districtCode && stateCode && countryCode) {
        ...
        console.log("Skipping block for record", index + 1, {
          blockCode,
          districtCode,
          stateCode,
          countryCode,
      ...
      if (
        ...
        console.log("Skipping village for record", index + 1, {
          villageCode,
          villageName,
          blockCode,
          districtCode,
      ...
      if ((index + 1) % 10000 === 0) {
        console.log(
          `📊 Processed ${index + 1}/${mappedRecords.length} records...`,
        );
      }
    });
    ...
    const countriesToInsert = Array.from(uniqueData.countries).map(
      (countryCode) => ({
    ...
    const districtsToInsert = Array.from(uniqueData.districts).map(
      (districtStr) => {
