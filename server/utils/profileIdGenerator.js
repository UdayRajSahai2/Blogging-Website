...
export const generateProfileId = async (domainId, fieldId, specialtyId) => {
  try {
    ...
    if (fieldId.startsWith('other:')) {
      ...
      if (!field) {
        throw new Error('Invalid field selected or not under the specified domain');
      }
    ...
    if (specialtyId.startsWith('other:')) {
      ...
      if (!specialty) {
        throw new Error('Invalid specialty selected or not under the specified field');
      }
...
export const getProfessionNamesFromProfileId = async (profileId) => {
  try {
    const { domainCode, fieldCode, specialtyCode, hasCustomDomain, hasCustomField, hasCustomSpecialty } = parseProfileId(profileId);
    ...
    return {
      domain: hasCustomDomain ? 'Custom Domain' : (domain ? domain.name : 'Unknown'),
      field: hasCustomField ? 'Custom Field' : (field ? field.name : 'Unknown'),
      specialty: hasCustomSpecialty ? 'Custom Specialty' : (specialty ? specialty.name : 'Unknown')
    };
