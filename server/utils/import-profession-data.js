...
export const importProfessionData = async () => {
  try {
      ...
      for (const minorGroup of majorGroup.minor_groups) {
        ...
        for (const broadGroup of minorGroup.broad_groups) {
          ...
          for (const detailedOccupation of broadGroup.detailed_occupations) {
            console.log(
              `      Processing detailed: ${detailedOccupation.detailed_name}`,
            );
            ...
            const [detailed, detailedCreated] = await Profession.findOrCreate({
              ...
              defaults: {
                ...
                level: 3,
                parent_id: specialty.profession_id,
                code: detailedOccupation.detailed_code
                  .replace("-", "")
                  .substring(6, 8), // Extract last 2 digits
              },
            });
    ...
    console.log(
      `🎉 Profession data import completed! Total records imported: ${totalImported}`,
    );
...
export const clearProfessionData = async () => {
  try {
    ...
    console.log(
      `🔄 Updated ${usersUpdated[0]} users to have null profession_id`,
    );
    ...
    const totalDeleted =
      detailedDeleted + specialtiesDeleted + fieldsDeleted + domainsDeleted;
