//server\services\customerIdFromLocation.js
import fs from "fs";
 ...
 */
export function generateCustomerIdFromLocation({
  country = "India",
  state,
  district,
  blockOrSub,
  ...
  if (!record) {
    throw new Error(
      "No matching village record found for the provided location",
    );
  }
  ...
  if (!blockCode && record.sub_district_code)
    blockCode = record.sub_district_code;
  ...
  const abbr = [
    ...
    (record.block_name || record.sub_district_name || "")
      .slice(0, 2)
      .toUpperCase(),
