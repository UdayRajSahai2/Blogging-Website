...
const ProfessionSelector = ({
  ...
  const loadDomains = async () => {
    try {
      setLoading(true);
      const response = await axios.get(`${PROFESSIONS_API}/domains`);
  ...
  const loadFields = async (domainId) => {
    try {
      setLoading(true);
      const response = await axios.get(`${PROFESSIONS_API}/fields/${domainId}`);
  ...
  const loadSpecialties = async (fieldId) => {
    try {
      ...
      const response = await axios.get(
        `${PROFESSIONS_API}/specialties/${fieldId}`,
      );
  ...
  const searchProfessions = async (query, level, setSuggestions) => {
    try {
      const response = await axios.get(`${PROFESSIONS_API}/search`, {
        params: { query, level },
      });
          ...
          {!useOtherField ? (
            <div className="space-y-2">
              <select
                ...
                disabled={
                  disabled || loading || !selectedDomain || useOtherDomain
                }
                className="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 disabled:bg-gray-100 disabled:cursor-not-allowed"
              >
          ...
          {!useOtherSpecialty ? (
            <div className="space-y-2">
              ...
              >
                ...
                {specialties.map((specialty) => (
                  <option
                    key={specialty.profession_id}
                    value={specialty.profession_id}
                  >
                    {specialty.name}
                  </option>
                ))}
            ...
            <div className="space-y-2">
              <div className="flex items-center space-x-2 mb-2">
                ...
                />
                <label
                  htmlFor="listed-specialty-radio"
                  className="text-sm text-gray-700"
                >
                  Choose from list
                </label>
                    ...
                    <div className="absolute z-10 w-full mt-1 bg-white border border-gray-300 rounded-md shadow-lg max-h-60 overflow-auto">
                      {specialtySuggestions.map((suggestion) => (
                        <div
                          key={suggestion.profession_id}
                          onClick={() =>
                            selectSuggestion(suggestion, "specialty")
                          }
                          className="px-3 py-2 hover:bg-gray-100 cursor-pointer text-sm"
                        >
                          {suggestion.name}
      ...
      {selectedDomain && selectedField && selectedSpecialty && (
        <div className="mt-4 p-3 bg-blue-50 border border-blue-200 rounded-md">
          <p className="text-sm text-blue-800">
            <strong>Selected Profession:</strong>
            <br />
            {useOtherDomain
              ? otherDomainText
              : domains.find((d) => d.profession_id == selectedDomain)
                  ?.name}{" "}
            →{" "}
            {useOtherField
              ? otherFieldText
              : fields.find((f) => f.profession_id == selectedField)?.name}{" "}
            →{" "}
            {useOtherSpecialty
              ? otherSpecialtyText
              : specialties.find((s) => s.profession_id == selectedSpecialty)
                  ?.name}
          </p>
        </div>
      )}
