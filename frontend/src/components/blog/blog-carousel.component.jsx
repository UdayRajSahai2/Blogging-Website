...
/* -------------------- MINI CAROUSEL -------------------- */
/* MOVED ABOVE (ONLY REQUIRED STRUCTURAL FIX) */

const MiniCarousel = ({ blogs, isFirstBlog = false }) => {
  if (!blogs?.length) return null;

  const [index, setIndex] = useState(0);
  const current = blogs[index] || blogs[0];

  // reset index when blog list changes
  useEffect(() => {
    setIndex(0);
  }, [blogs]);
    ...
    >
      ...
      <div
        className="absolute top-2 left-2 sm:top-3 sm:left-4 z-10 text-[10px] sm:text-xs bg-black/60 text-white px-2 py-[2px] sm:px-3 sm:py-1
                  rounded-md font-semibold capitalize tracking-wide"
      >
        {tag}
      </div>
