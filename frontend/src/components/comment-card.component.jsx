...
const CommentCard = ({
  ...
  const [totalRepliesFromServer, setTotalRepliesFromServer] = useState(
    total_replies || children.length,
  );
  ...
  const loadReplies = async (skip = 0, append = false) => {
    ...
    try {
      const response = await axios.post(`${COMMENT_API}/get-comment-replies`, {
        comment_id,
        skip,
      });
  ...
  const displayReplyCount = repliesVisible
    ? replies.length
    : totalRepliesFromServer;
  const hasMoreReplies = repliesVisible
    ? replies.length < totalRepliesFromServer
    : totalRepliesFromServer > 0;
  ...
  return (
    <div
      className="w-full"
      style={{ paddingLeft: `${Math.min(leftval * 8, 32)}px` }}
    >
      <div
        className={`my-4 p-5 rounded-xl border transition-all duration-200 hover:shadow-md ${
          isReply
      ...
      >
        ...
        <div className="flex justify-between items-start mb-4">
          <div className="flex gap-3 items-center">
            ...
            />
            <div>
              <p className="font-medium text-sm line-clamp-1">{fullname}</p>
        ...
        <div className="flex gap-4 items-center ml-2">
          ...
          {!shouldShowReplies && displayReplyCount > 0 && (
            <span className="text-xs text-dark-grey italic">
              Maximum reply depth reached • {displayReplyCount}{" "}
                ...
                >
                  {loadingMoreReplies ? (
                    ...
                    <>
                      ...
                      Load more replies (
                      {totalRepliesFromServer - replies.length} remaining)
                    </>
                  )}
                </button>
              </div>
